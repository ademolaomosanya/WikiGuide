...
import { GuidedEditsPage } from "./GuidedEditsPage";
...
export function DashboardPage({ user, authIsLoading }: DashboardPageProps) {
  ...
  if (authIsLoading) {
    return <Status message={t("Checking your session…")} />;
  }

  if (!user) {
    return (
      <main className="dashboard-auth-gate">
        <p className="dashboard-kicker">{t("Personal learning path")}</p>
        <h1>{t("Sign in to start your contributor journey.")}</h1>
        <p>
          {t("Your Wikimedia identity keeps lesson progress, XP, and contribution statistics attached to the right account.")}
        </p>
  ...
  if (isLoading) return <Status message={t("Building your learning path…")} />;
  if (!flow) return <Status message={error ?? t("Your learning path is unavailable.")} />;
  if (onboarding.isLoading) return <Status message={t("Preparing your welcome…")} />;
  ...
  return (
    <main className="learning-shell">
      <aside className="learning-sidebar" aria-label={t("Dashboard sections")}>
      ...
      <div className="learning-workspace">
        <header className="learning-topbar">
          <div>
            <p className="dashboard-kicker">{t("Welcome back")}</p>
            <strong>{flow.username}</strong>
          </div>
          <div className="learning-quick-stats" aria-label={t("Learning summary")}>
            <span><b>{flow.stats.totalPoints}</b> XP</span>
            <span><b>{flow.stats.completedLessons}</b> {language === "fr" ? "leçons" : "lessons"}</span>
            <span><b>{flow.stats.wikimediaEdits.toLocaleString()}</b> {language === "fr" ? "modifications" : "edits"}</span>
          </div>
        </header>
        ...
        {section === "gamification" && (
          <GamificationSection
            leaderboard={flow.leaderboard}
            quests={flow.quests}
            totalPoints={flow.stats.totalPoints}
          />
        )}
        {section === "mentor" && (
          <MentorSection
  ...
  return (
    <section className="workflow-section">
      <div className="workflow-heading">
        <div>
          <p className="dashboard-kicker">{t("Contributor curriculum")}</p>
          <h1>{t("Choose a Wikimedia learning path")}</h1>
        </div>
        <span>{activePath.completedLessons}/{activePath.totalLessons} {language === "fr" ? "terminées" : "complete"}</span>
      </div>
          ...
          {activePath.lessons.map((lesson) => (
            <li className={`lesson-step is-${lesson.state}`} key={lesson.slug}>
              <button
                ...
                aria-label={`${t(lesson.title)}, ${t(lesson.state)}`}
              ...
              <div>
                ...
                <span>{lesson.state === "completed" ? t("Completed") : lesson.state === "locked" ? t("Locked") : `${lesson.points} XP`}</span>
              </div>
            </li>
          ))}
        </ol>
          ...
          {selectedLesson ? (
            <>
              <span className={`lesson-state is-${selectedLesson.state}`}>
                {t(selectedLesson.state)}
              </span>
              <p className="dashboard-kicker">{t("Lesson")} {selectedLesson.order}</p>
              <h2>{t(selectedLesson.title)}</h2>
              <p>{t(selectedLesson.description)}</p>
              {selectedLesson.state === "available" && (
                ...
                >
                  {completingSlug === selectedLesson.slug
                    ? t("Saving progress…")
                    : `${t("Complete lesson")} · +${selectedLesson.points} XP`}
                </button>
              )}
              {selectedLesson.state === "completed" && <strong className="lesson-done">{t("Lesson complete")}</strong>}
            </>
          ) : (
            <p>{t("Select an unlocked lesson to begin.")}</p>
          )}
        </aside>
      </div>
    </section>
...
function GamificationSection({
  ...
  const currentRank = leaderboard.entries.find((entry) => entry.isCurrentUser)?.rank;

  return (
    <section className="workflow-section gamification-section">
      <div className="workflow-heading">
        <div>
          <p className="dashboard-kicker">{t("Progress and challenges")}</p>
          <h1>{t("Gamification")}</h1>
        </div>
      </div>

      <div className="gamification-summary" aria-label="Gamification summary">
        <span><b>{totalPoints}</b> {language === "fr" ? "XP gagnés" : "XP earned"}</span>
        <span><b>{completedQuests}/{quests.length}</b> {language === "fr" ? "quêtes terminées" : "quests complete"}</span>
        <span>
          <b>{leaderboard.unlocked ? currentRank ? `#${currentRank}` : "—" : "Locked"}</b>
          {t("leaderboard")}
        </span>
      </div>
      ...
      <div className="gamification-block">
        <div className="gamification-block-heading">
          <div>
            <p className="dashboard-kicker">{t("Play by contributing")}</p>
            <h2>{t("Find a suggested edit")}</h2>
          </div>
          <span>{t("Live tasks from French Wikipedia")}</span>
        </div>
        <GuidedEditsPage />
      </div>
...
function LeaderboardSection({
  ...
  return (
    <section className="workflow-section compact">
      <div className="workflow-heading">
        <div><p className="dashboard-kicker">{t("Community")}</p><h1>{t("Leaderboard")}</h1></div>
      </div>
      {!leaderboard.unlocked ? (
        <div className="locked-feature">
          <span aria-hidden="true">◆</span>
          <h2>{t("Unlock the leaderboard")}</h2>
          <p>{t("Complete one lesson to compare your XP with other WikiGuide learners.")}</p>
        </div>
      ) : (
        <ol className="leaderboard-list">
          {leaderboard.entries.map((entry) => (
            <li className={entry.isCurrentUser ? "is-current" : ""} key={`${entry.rank}-${entry.username}`}>
              ...
              <div><strong>{entry.username}</strong><span>{entry.completedLessons} {language === "fr" ? "leçons" : "lessons"}</span></div>
...
function QuestsSection({
  ...
  return (
    <section className="workflow-section compact">
      <div className="workflow-heading"><div><p className="dashboard-kicker">{t("Milestones")}</p><h1>{t("Your quests")}</h1></div></div>
...
function MentorSection({
  ...
  return (
    <section className="workflow-section compact">
      <div className="workflow-heading">
        <div>
          <p className="dashboard-kicker">{t("Guidance from people")}</p>
          <h1>{t("Match with a mentor")}</h1>
        </div>
      </div>

      {isLoading ? (
        <p className="section-loading">{t("Loading your matching status…")}</p>
      ) : request && request.status !== "closed" ? (
        <article className={`mentor-status is-${request.status}`}>
          <span aria-hidden="true">{request.status === "matched" ? "✓" : "…"}</span>
          <div>
            <p className="dashboard-kicker">{request.status === "matched" ? t("Match ready") : t("Request received")}</p>
            <h2>{request.projectName} · {request.topicName}</h2>
            <p>
              {request.status === "matched"
                ? t("Your request has been matched. A coordinator can now connect you with the assigned mentor.")
                : t("Your request is waiting for an appropriate volunteer mentor. WikiGuide will not claim a match until one is actually assigned.")}
            </p>
            <small>{t("Submitted")} {new Date(request.created_at).toLocaleDateString(language)}</small>
          </div>
        </article>
      ) : (
        <form className="mentor-form" onSubmit={handleSubmit}>
          <p>
            {t("Tell us where you need help. This creates a matching request; it does not guarantee immediate volunteer availability.")}
          </p>
          <div className="mentor-form-grid">
            <label>
              <span>{t("Wikimedia project")}</span>
            ...
            <label>
              ...
              <select value={topic} onChange={(event) => setTopic(event.target.value)}>
                <option value="getting-started">{t("Getting started")}</option>
                <option value="editing">{t("Editing and formatting")}</option>
                <option value="sources">{t("Sources and citations")}</option>
                <option value="community">{t("Community participation")}</option>
                <option value="technical">{t("Technical contribution")}</option>
              </select>
            </label>
            <label>
              <span>{t("Your experience")}</span>
              <select value={experience} onChange={(event) => setExperience(event.target.value)}>
                <option value="new">{t("New contributor")}</option>
                <option value="beginner">{t("Some contribution experience")}</option>
                <option value="experienced">{t("Experienced contributor")}</option>
              </select>
            </label>
            <label className="is-wide">
              <span>{t("What would you like to achieve?")} <small>{t("Optional")}</small></span>
              <textarea
                maxLength={500}
                placeholder={t("For example: I want help adding my first reliable citation.")}
                value={goals}
          ...
          <button disabled={isSubmitting} type="submit">
            {isSubmitting ? t("Submitting request…") : t("Request a mentor match")}
          </button>
        </form>
      )}
    </section>
...
function CommunitiesSection({ communities }: { communities: WikimediaCommunity[] }) {
  ...
  return (
    <section className="workflow-section compact">
      <div className="workflow-heading">
        <div>
          <p className="dashboard-kicker">{t("Wikimedia is collaborative")}</p>
          <h1>{t("Connect with communities")}</h1>
        </div>
      </div>
      <p className="communities-intro">
        {t("These links open official public Wikimedia discussion spaces. Read each space’s guidance before posting, and remember that your contributions will be public.")}
      </p>
          ...
          <article className={`community-card theme-${community.projectSlug}`} key={community.url}>
            ...
            <div>
              ...
              <a href={community.url} rel="noreferrer" target="_blank">
                {t("Visit official community")} <span aria-hidden="true">↗</span>
              </a>
            </div>
          </article>
        ))}
  ...
  return (
    <section className="workflow-section compact">
      <div className="profile-card">
        ...
        <p className="dashboard-kicker">{t("Wikimedia learner")}</p>
        <h1>{username}</h1>
        <div className="profile-stats">
          <span><b>{stats.totalPoints}</b> {language === "fr" ? "XP gagnés" : "XP earned"}</span>
          <span><b>{stats.completedLessons}</b> {language === "fr" ? "leçons terminées" : "lessons completed"}</span>
          <span><b>{stats.wikimediaEdits.toLocaleString()}</b> {language === "fr" ? "modifications Wikimedia" : "Wikimedia edits"}</span>
        </div>
      </div>
    </section>
  );
